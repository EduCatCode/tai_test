...
def get_disaster_info(scenario_name):
    scenarios = {
        ...
        'earthquake': {
            'title': '🔴 地震發生',
            'description': '偵測到強烈地震！震央位於成功大學附近，規模 6.5',
            'color': 'error',
            'actions': [
                '立即尋找掩護，遠離窗戶',
                '關閉瓦斯、電源',
        ...
        'flooding': {
            'title': '🌊 淹水警報',
            'description': '豪雨造成多處淹水，低窪地區水深超過 50 公分',
            'color': 'warning',
            'actions': [
                '遠離淹水區域',
                '往高處避難',
        ...
        'air_pollution': {
            'title': '🏭 嚴重空氣污染',
            'description': '工業區異常排放，PM2.5 濃度暴增至危險等級',
            'color': 'error',
            'actions': [
                '關閉門窗',
                '開啟空氣清淨機',
        ...
        with col_video:
            ...
            if not gif_displayed:
                ...
                <div style="background-color: #ffe6e6; padding: 20px; border-radius: 10px; text-align: center;">
                    <h3>📹 現場監控</h3>
                    <p>將 GIF 檔案命名為 <code>output.gif</code><br/>放在與程式相同的目錄即可顯示</p>
                </div>
                """, unsafe_allow_html=True)
        
        with col_map:
            st.subheader("📍 地震震度分布圖")
    ...
    if scenario != 'normal':
        ...
        if use_persona:
            ...
            with col_message:
                ...
                if disaster_info['actions']:
                    st.markdown("**請立即執行:**")
                    for action in disaster_info['actions'][:3]:
